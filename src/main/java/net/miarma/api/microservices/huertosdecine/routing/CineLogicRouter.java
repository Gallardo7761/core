...
public class CineLogicRouter {
    public static void mount(Router router, Vertx vertx, Pool pool) {
        ...
        router.get(CineEndpoints.SELF_VOTE).handler(AuthGuard.check()).handler(hVoteLogic::getVoteSelf);
        ...
        router.put(CineEndpoints.MOVIE_VOTES).handler(AuthGuard.check()).handler(hVoteLogic::updateVote);
